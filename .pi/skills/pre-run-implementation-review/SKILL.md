...
- After an external termination, rerun the same `reviewer_job.py` command. The runner detects a `running` state whose transport child is dead, records `service_failed`, and resumes the recorded session instead of creating a second scientific opinion. The dead-child detection inspects the Linux process table (`ps`); on platforms without it the runner conservatively assumes the review is alive and falls back to the bounded attempt timeout, so a stuck job there resolves via timeout rather than detection.
