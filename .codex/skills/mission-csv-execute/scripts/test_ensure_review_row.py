...
PROJECT_EXTRA_FIELDS = [
    "spec_id", "exp_id", "run_id", "remote_state", "artifact_path",
    "branch", "commit_hash", "next_action", "updated_at",
]
PROJECT_FIELDNAMES = FIELDNAMES + PROJECT_EXTRA_FIELDS
...
class EnsureReviewRowTests(unittest.TestCase):
    def write_csv(self, path: Path, rows: list[dict], fieldnames: list[str] = FIELDNAMES) -> None:
    ...
    def test_appends_review_without_inventing_sidecars(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("review_requested_model:pending", review["notes"])
            self.assertIn("scientific_outcome:pending", review["notes"])
    ...
    def test_appends_review_to_project_28_column_csv(self) -> None:
        module = load_module()
        row = dict.fromkeys(PROJECT_FIELDNAMES, "")
        row.update(self.ordinary_row())
        row["spec_id"] = "SPEC-01"
        row["remote_state"] = "not_applicable"
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "project.csv"
            self.write_csv(path, [row], PROJECT_FIELDNAMES)
            self.assertTrue(module.ensure_review_row(path))
            with path.open(encoding="utf-8-sig", newline="") as handle:
                reader = csv.DictReader(handle)
                rows = list(reader)
            self.assertEqual(reader.fieldnames, PROJECT_FIELDNAMES)
            self.assertEqual([item["id"] for item in rows], ["TASK-01", "REVIEW-01"])
            self.assertEqual(rows[-1]["remote_state"], "not_applicable")
