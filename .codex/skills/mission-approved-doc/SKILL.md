...
- `PRERUN-REVIEW-*` 是运行前代码实施审查，不是逐 issue 审查；不要在每个小改动、每个文件或每条普通 row 后插入
- `PRERUN-REVIEW-*` 的 `required_skills` 固定写 `pre-run-implementation-review`
- 若同一个 CSV 中存在多轮“新代码快照 → 训练 / 运行”，每轮首次运行前都必须有对应的 `PRERUN-REVIEW-N`；若审查后又修改了影响运行的代码，原 pre-run review 失效，必须新增或重跑 pre-run review
- `PRERUN-REVIEW-*` 记录的是本次训练 / 运行使用的 `pre_run_code_commit`，不是整个 CSV 的最终 commit；后续 artifact、ingest、analysis、review 或修复 commit 必须另行记录
...
- 若当前环境支持同模型 sub-agent，优先使用独立 sub-agent；若不支持，必须在 notes / review log 中记录受限 fallback review
