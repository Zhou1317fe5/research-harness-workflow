...
1. **Capture Failure Evidence**
   - Prefer the simplest cheap reproduction.
   - Automated regression test if practical.
   - One-off script, failing command, stack trace, log, or remote artifact is acceptable when it precisely captures the bug.
   - Use `superpowers:test-driven-development` only when the user or approved task explicitly requests TDD/RED.
   - Do not run extra expensive remote training/eval just to manufacture RED.
...
| "I'll write test after confirming fix works" | Capture failure evidence first when it is cheap; otherwise document the existing log/artifact that proves the bug. |
...
| **4. Implementation** | Capture failure evidence, fix, verify | Bug resolved, relevant verification passes |
...
**Related skills:**
- **superpowers:test-driven-development** - Only for explicit user/task TDD/RED requests
