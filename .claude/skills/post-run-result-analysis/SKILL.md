...
- 正式分析必须由独立的强模型 reviewer 完成，模型取 `review_model.model_for_host("<宿主>")` 的当前值、thinking 固定 `high`；两条等价通道按宿主选择：
  模型的 canonical 值与运行时匹配正则统一定义在 `.agents/harness/review_model.py`；额度耗尽等原因需要切换时改该文件（一次显式提交），验证器与各 runner 自动跟随，不允许会话内临时传参绕过。 同一逻辑模型在不同宿主的注册名可不同（Pi 用 provider 前缀名，codex CLI 用裸名），`review_model.py` 的 `MODELS` 映射按宿主给出调用名；索引与 CSV 记录统一用 canonical（Pi 侧）名字，验证器接受任一宿主注册名加 thinking 后缀的运行时观测值。
  ...
  2. **codex-exec 通道（`codex-exec-independent`）**：宿主无 sub-agent 派发能力时，用 `scripts/run_result_analysis.py` 启动 fresh、ephemeral、read-only 的独立 `codex exec` 会话（模型取自 `review_model.model_for_host("codex")`）。
...
- 请求模型取 `.agents/harness/review_model.py` 的当前 Pi 值、thinking `high`；
