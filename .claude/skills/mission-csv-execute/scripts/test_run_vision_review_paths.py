...
class RunVisionReviewPathTests(unittest.TestCase):
    ...
    def test_vision_met_allows_negative_scientific_outcome_without_execution_gap(self) -> None:
        module = load_module()
        result = valid_result(
            scientific_outcome="gate_failed",
            outcome_answers=[{
                "question_id": "OUTCOME-001",
                "verdict": "not_run",
                "answer": "The preregistered gate stopped the later stage.",
                "evidence_refs": ["results.json:G2=false"],
                "confidence": "high",
                "boundary": "The later-stage hypothesis was not evaluated.",
                "next_action": "None; this is the approved terminal branch.",
            }],
        )
        contract = {"reader_questions": [{"id": "OUTCOME-001", "question": "First?"}]}
        self.assertEqual(module.validate_review_result(result, contract), [])
    ...
    def test_legacy_review_without_scientific_outcome_remains_readable(self) -> None:
        module = load_module()
        result = valid_result()
        result.pop("scientific_outcome")
    ...
    def test_evidence_close_uses_no_review_model(self) -> None:
        module = load_module()
        result = valid_result(
    ...
    def test_only_canonical_review_modes_are_accepted(self) -> None:
        module = load_module()
        expected = {
        ...
        for mode, normalized in expected.items():
            with self.subTest(mode=mode):
                self.assertEqual(module.normalize_review_mode(mode), normalized)
        for removed in (
            "direct-spawn-agent",
            "codex-exec-subagent",
            ...
            with self.subTest(removed=removed):
                with self.assertRaises(ValueError):
                    module.normalize_review_mode(removed)
