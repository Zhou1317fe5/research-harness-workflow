...
def _nonempty_evidence(value: object) -> bool:
    ...
    *,
    ...
    for index, claim in enumerate(claims):
        ...
        if status == "not_run_by_preregistered_gate" and not _nonempty_evidence(
            ...
            errors.append(
                f"{claim_id} not_run_by_preregistered_gate requires gate_evidence"
            )
        if (
            status == "verified"
            and claim.get("evidence_required") == "real_e2e"
            and not _nonempty_evidence(claim.get("evidence_refs"))
        ):
            errors.append(f"{claim_id} real_e2e verified requires evidence_refs")
