    ...
    row.update(
        {
            ...
            "acceptance_criteria": "Every ingested ExpID/RunID is covered by reviews/result-analysis.json; each analysis.md has Change/Result/Finding/Next, evidence refs, SHA-256, per-entry reviewer evidence and output SHA-256, and a verifiable scientific outcome; strong reviewer identity is recorded from parent session/tool evidence; no Executor self-review fallback.",
            "test_mcp": "contract",
            "required_skills": "post-run-result-analysis",
            "required_mcp": "",
            "review_initial_requirements": "Verify the independent scientific-reviewer sub-agent input is raw evidence rather than the Executor's conclusion.",
            "review_regression_requirements": "Verify analysis index coverage, model evidence, analysis hashes, evidence refs and fixed four-section analysis before REVIEW-*.",
            "dev_state": "未开始",
