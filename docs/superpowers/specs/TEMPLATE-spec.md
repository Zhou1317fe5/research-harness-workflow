...
| Wrapper | scripts/train_eval_shutdown_autodl.sh |
...
- **Success Signal**：<具体指标 + 阈值 + split。示例：val <主指标> ≥ 当前 baseline + 1.0（fold0 1-shot, seed 0）。>
- **Failure Signal**：<**独立定义**，不是"未达到 Success"的反面。示例：NaN / OOM / <指标反向恶化阈值> / <持续低于预期阈值>。必须事先想清楚"什么样的现象会让你判定方法失败"。>
- **Metric & Split**：<<主指标> / <辅助指标>；1-shot / 5-shot；fold0 / fold1 / ... ；val / test；seed 列表。>
- **Stop Condition**：
...
- 实验记录：`research_workspace/00-实验记录.md` 索引行 + `research_workspace/experiments/<ExpID>/` 详细解读。
