...
工作流说明见 [面向科研的 harness engineering](面向科研的harness-engineering.md)。
...
`.agents/harness/` 按功能组织公共实现；各子目录的职责见 [目录说明](harness-layout.md)。旧版根目录 `scripts/`、`.claude/harness/`、
...
按 [用户配置说明](configuration.md) 接入；配置统一放在 `.agents/harness/config/`。
