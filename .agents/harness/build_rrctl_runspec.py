...
DEFAULT_ADAPTER_ARGV = ["python", ".agents/harness/rrctl_adapters/generic_json.py"]
EXECUTION_PURPOSES = {
    "official",
    "pilot",
    "pre_review_smoke",
...
def _load_project_adapters() -> dict[tuple[str, ...], dict[str, Any]]:
    ...
    ``.agents/harness/rrctl_project_adapters.py``，模板化时删除该文件即可。
    """
    try:
        # 执行时项目根在 sys.path，.agents/harness/ 不是包，用裸导入
        import sys
        from pathlib import Path
        harness = Path(__file__).parent
        if str(harness) not in sys.path:
            sys.path.insert(0, str(harness))
        from rrctl_project_adapters import PROJECT_ADAPTERS
    except Exception:
        return {}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.project_config:
            from project_config import apply_project_config
            try:
