...
Copy the shape in `examples/profiles.example.json` to `~/.config/rrctl/profiles.json` and set mode `0600` when a password environment reference is present. RunSpec files contain only the profile name, never credentials.
